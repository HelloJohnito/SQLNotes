...
def heart_tracks
  ...
  execute(<<-SQL)
  SELECT albums.title, COUNT(tracks.album)
  FROM albums
  JOIN tracks ON tracks.album = albums.asin
  WHERE song LIKE '%Heart%'
  GROUP BY albums.title, tracks.album
  ORDER BY COUNT(tracks.album) DESC, albums.title;
  SQL
end
...
def rock_superstars
  ...
  execute(<<-SQL)
  SELECT artist, COUNT(DISTINCT albums)
  FROM albums
